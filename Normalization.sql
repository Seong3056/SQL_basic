...
-- > 일반적으로 데이터베이스의 설계 단계에서 진행하고, 
-- > 경우에 따라서 기존 시스템을 재검토할 때 정규화 하는 경우도 있음
...
-- 제1 정규형
-- > 관계형 데이터배이스의 테이블에는 ㅅ하나의 셀에 하나의 값만 저장할 수 있다는 제약이 있음
-- > 따라서 주문상품 데이터를 그대로 테이블로 만들 수 없음
-- -- 상품 코드, 상품명, 개수 의 3개의 열로 나누어야함

-- > 제1 졍규화를 수행하면 하나의 셀에 하나의 값만 저장하게 되므로 정상적으로 테이블화 할 수 있음
...
-- -- 예제 상황에서는 한번의 주문으로 여러 개의 상품을 주문할 수 있으므로
-- -- 주문번호, 날짜, 성명, 연락처가 동일한 값을 가지는 행이 여러개 만들어짐
-- -- 이때 동일한 값을 가지는 중복 행이 여러 개 존재하지 않도록 정리

-- 테이블 분할을 하면 반복되는 부분이 하나로 정리되고, 주문 데이터가 변경되면 주문 테이블 하나만 수정하면 됨
-- > 분할 전 데이터를 원하더라도 주문번호로 JOIN하면 되므로 문제 없음

-- 분할 이후의 주문 테이블은 주문 번호에 중복된 값이 없기 때문에 기본키로 지정할 수 있음
-- > 주문 상품 테이블에서는 주문 번호와 상품코드 하나로 묶어 기본키로 지정할 수 있음
